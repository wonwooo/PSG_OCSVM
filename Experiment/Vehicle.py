...
def main(generation_type):
    ...
    _, pval_f1 = stats.ttest_1samp(scores_Vehicle_macro, 0.651)
    _, pval_MCC = stats.ttest_1samp(MCC, 0.542)
    with open('TestResult.txt', 'a') as f:
        f.write('Vehicle : ' + str(np.mean(scores_Vehicle_macro)) + ', p_f1 : ' + str(pval_f1) + \
                ', MCC : ' + str(np.mean(MCC)) + ', p_MCC : ' + str(pval_MCC) + '\n')
    print('Vehicle : ', np.mean(scores_Vehicle_macro))