...
def main(generation_type):
    print('=========Testing SVMguide1 dataset=========')
    df = pd.read_csv('datasets/svmguide1.txt', header=None, sep=' ')
    classes = df[0]
    df = df.drop(0, axis=1)
    val = []
    ...
    _, pval_f1 = stats.ttest_1samp(scores_SVMguide1_macro, 0.838)
    _, pval_MCC = stats.ttest_1samp(MCC, 0.764)
    with open('TestResult.txt', 'a') as f:
        f.write('SVMguide1 : ' + str(np.mean(scores_SVMguide1_macro)) + ', p_f1 : ' + str(pval_f1) + \
                ', MCC : ' + str(np.mean(MCC)) + ', p_MCC : ' + str(pval_MCC) + '\n')

    print('SVMguide1 : ', np.mean(scores_SVMguide1_macro))