...
def main(generation_type):
    print('=========Testing Australian dataset=========')
    ...
    for c in classes:
        for iter_ in range(10):
            ...
            _, pval_f1 = stats.ttest_1samp(scores_Australian_macro, 0.592)
            _, pval_MCC = stats.ttest_1samp(MCC, 0.335)
    with open('TestResult.txt', 'a') as f:
        f.write('Australian : ' + str(np.mean(scores_Australian_macro)) + 'p-value on f1 : ' + str(pval_f1) + \
                ' MCC : ' + str(np.mean(MCC)) + 'p-value on MCC : ' + str(pval_MCC) + '\n')
    print('Australian : ' + str(np.mean(scores_Australian_macro)) + 'MCC : ' + str(np.mean(MCC)) + '\n')
