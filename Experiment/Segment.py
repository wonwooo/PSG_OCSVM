...
def main(generation_type):
    ...
    _, pval_f1 = stats.ttest_1samp(scores_Segment_macro, 0.769)
    _, pval_MCC = stats.ttest_1samp(MCC, 0.644)
    with open('TestResult.txt', 'a') as f:
        f.write('Segment : ' + str(np.mean(scores_Segment_macro)) + ', p_f1 : ' + str(pval_f1) + \
                ', MCC : ' + str(np.mean(MCC)) + ', p_MCC : ' + str(pval_MCC) + '\n')
    print('Segment : ', np.mean(scores_Segment_macro))