...
def test(generation_type):
    ...
    _, pval_f1 = stats.ttest_1samp(scores_Glass_macro, 0.577)
    _, pval_MCC = stats.ttest_1samp(MCC, 0.495)
    with open('TestResult.txt', 'a') as f:
        f.write('Glass : ' + str(np.mean(scores_Glass_macro)) + ', p_f1 : ' + str(pval_f1) + \
                ', MCC : ' + str(np.mean(MCC)) + ', p_MCC : ' + str(pval_MCC) + '\n')
    print('Glass : ', np.mean(scores_Glass_macro))