...
def test(generation_type):
    ...
    _, pval_f1 = stats.ttest_1samp(scores_Heart_macro, 0.567)
    _, pval_MCC = stats.ttest_1samp(MCC, 0.282)
    with open('TestResult.txt', 'a') as f:
        f.write('Heart : ' + str(np.mean(scores_Heart_macro)) + ', p_f1 : ' + str(pval_f1) + \
                ', MCC : ' + str(np.mean(MCC)) + ', p_MCC : ' + str(pval_MCC) + '\n')
    print('Heart : ', np.mean(scores_Heart_macro))
