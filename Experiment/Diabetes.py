...
def test(generation_type):
    ...
    df = pd.read_csv('diabetes.txt', header=None, sep=' ')
    df = df.drop(1, axis=1)
    ...
    _, pval_f1 = stats.ttest_1samp(scores_Diabetes_macro, 0.508)
    _, pval_MCC = stats.ttest_1samp(MCC, 0.168)
    with open('TestResult.txt', 'a') as f:
        f.write('Diabetes : ' + str(np.mean(scores_Diabetes_macro)) + ', p_f1 : ' + str(pval_f1) + \
                ', MCC : ' + str(np.mean(MCC)) + ', p_MCC : ' + str(pval_MCC) + '\n')
    print('Diabetes : ', np.mean(scores_Diabetes_macro))