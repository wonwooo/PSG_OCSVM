...
def main(generation_type):
    ...
    _, pval_f1 = stats.ttest_1samp(scores_Sonar_macro, 0.506)
    _, pval_MCC = stats.ttest_1samp(MCC, 0.141)
    with open('TestResult.txt', 'a') as f:
        f.write('Sonar : ' + str(np.mean(scores_Sonar_macro)) + ', p_f1 : ' + str(pval_f1) + \
                ', MCC : ' + str(np.mean(MCC)) + ', p_MCC : ' + str(pval_MCC) + '\n')
    print('Sonar : ', np.mean(scores_Sonar_macro))