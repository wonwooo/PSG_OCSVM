...
def test(generation_type):
    ...
    for c in classes:
        for iter_ in range(10):
            ...
            scores_Vote_macro.append(mean_score_macro)
            scores_Vote_micro.append(mean_score_micro)
            scores_Vote_weighted.append(mean_score_weighted)

            mcc1 = matthews_corrcoef(y_true1, y_pred1)
            mcc2 = matthews_corrcoef(y_true2, y_pred2)
            MCC.append(np.mean([mcc1, mcc2]))

    _, pval_f1 = stats.ttest_1samp(scores_Vote_macro, 0.733)
    _, pval_MCC = stats.ttest_1samp(MCC, 0.540)
    with open('TestResult.txt', 'a') as f:
        f.write('Vote : ' + str(np.mean(scores_Vote_macro)) + ', p_f1 : ' + str(pval_f1) + \
                ', MCC : ' + str(np.mean(MCC)) + ', p_MCC : ' + str(pval_MCC) + '\n')

    print('Vote : ', np.mean(scores_Vote_macro))