...
class PseudoPoints:
    ...
    def visualize_ball(self, index_of_target):
        ...
        ax.scatter(np.mean(pointset.iloc[:,0]),np.mean(pointset.iloc[:,1]), s=30, color = 'yellow', label='centroid of neighbors')
    ...
    def get_features(self):
        ...
        for i in range(len(self.data)):
            ...
            if len(idx) != 1:
                pseudo_outlier = targetcoord - np.mean(pointset.iloc[:, 0:self.dim])
                pseudo_outlier = self.epsilon * (pseudo_outlier / np.linalg.norm(pseudo_outlier)) + targetcoord
            else:
        ...
        return f1, f2, f3,  np.array(outlier_candidates), np.array(target_candidates)
    ...
    def get_pseudo_points(self):
        ...
        for i in range(self.testNum):
            ...
            accepted_outliers = outlier_candidates[prob > u]
            accepted_targets = target_candidates[prob < u]
            pseudo_outliers = np.append(pseudo_outliers, accepted_outliers, axis=0)
            pseudo_targets = np.append(pseudo_targets, accepted_targets, axis=0)

        nearest_mean = np.mean([np.sort(np.sqrt(np.sum((i - np.array(self.data)) ** 2, axis=1)))[3] for i in np.array(self.data)])
    ...
    def showPseudoPoints(self):
        ...
        if self.dim==2:
            plt.scatter(dataWithProb.iloc[:, 0], dataWithProb.iloc[:, 1],  c=dataWithProb['prob'], cmap='Blues', label='_nolegend_')
    ...
    def search_optimal_hyperparameters(self):
        ...
        if self.dim ==2:
            plt.scatter(dataWithProb.iloc[:, 0], dataWithProb.iloc[:, 1], c=dataWithProb['prob'], cmap='Blues', label='originaldata', s=20)
            plt.scatter(np.array(pseudo_outliers)[:, 0], np.array(pseudo_outliers)[:, 1], color='red', marker='x',
                        label='pseudo_outliers', s=20)
            plt.scatter(np.array(pseudo_targets)[:, 0], np.array(pseudo_targets)[:, 1], color='green', marker='*',
                        label='pseudo_targets', s=20)
